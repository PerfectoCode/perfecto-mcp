...
class TestTracebackSanitization:
    ...
    def test_sanitized_traceback_does_not_redact_url_paths(self):
        # URL path segments like /api/v1/devices should not be mangled
        try:
            raise RuntimeError("Request failed for url 'https://demo.app.perfectomobile.com/api/v1/devices'")
        except RuntimeError as exc:
            sanitized = format_sanitized_traceback(exc)

        assert "/api/v1/devices" in sanitized
