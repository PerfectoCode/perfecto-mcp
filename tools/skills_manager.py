...
- list_skill_resources: List all the Skills Resources available to learn.
    args(dict): Dictionary with the following required parameters:
        skill_name (str): The skill name.
- read_skill_resource_uri: Read file content based on a Skill Resource URI (perfecto-skill-{skill_name}://{resource_path}).
    args(dict): Dictionary with the following required parameters:
        skill_resource_uri (str): The skill URI.
- read_skill_resource_uri_list: Read file content based on a Skill Resource URI list (['perfecto-skill-{skill_name}://{resource_path}', ...]).
    args(dict): Dictionary with the following required parameters:
